...
def _mock_authenticate_user(_, client=None):
    u"""Mock Pycognito authenticate user method. This code is from Pycognito's test suite."""
    return {
        u"AuthenticationResult": {
            u"TokenType": u"admin",
            u"IdToken": u"dummy_token",
            u"AccessToken": u"dummy_token",
            u"RefreshToken": u"dummy_token",
        }
    }

def _mock_verify_tokens(self, token, id_name, token_use):
    u"""Mock Pycognito verify tokens method. This code is from Pycognito's test suite."""
    if u"wrong" in token:
...
class CloudServerTestCase(unittest.TestCase):
    ...
    @mock.patch(u'pycognito.aws_srp.AWSSRP.authenticate_user', _mock_authenticate_user)
    @mock.patch(u'pycognito.Cognito.verify_token', _mock_verify_tokens)
    def test_login(self, mock_request):
        u"""Test that successful login returns a bearer token"""

        mock_response = mock.Mock()
        mock_response.json.return_value = {u'response': {u'token': u'dummytoken'},
                                           u'messages': [{u'code': u'0', u'message': u'OK'}]}
        ...
        self.assertIsInstance(self._fms._token, unicode)
    ...
    def test_non_ssl_handling(self):
        ...
        with self.assertRaises(ValueError):
            fmrest.CloudServer(url=u"http://127.0.0.1",
                               user=ACCOUNT_NAME,
                               password=ACCOUNT_PASS,
                               database=DATABASE,
                               layout=LAYOUT
